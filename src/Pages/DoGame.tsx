...
const DoGame = () => {
    ...
    const { game, name, answers, index }: IGuestInfo = useSelector((state: IRootState) => state.guest);

    const [formerSelected, setFormerSelected] = useState(false);
    const [latterSelected, setLatterSelected] = useState(false);

    const mapNumber = (count: number, index: number) => {
        const numbers = [];
    ...
    const postGuest = async (updated: number[]) => {
